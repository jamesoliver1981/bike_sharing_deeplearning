...
class NeuralNetwork(object):
    ...
    def forward_pass_train(self, X):
        ...
        hidden_inputs = X[:,None] # signals into hidden layer
        
        hidden_outputs = np.dot(self.weights_input_to_hidden.T,hidden_inputs) # signals from hidden layer
...
#         weights_h_o_T=self.weights_hidden_to_output.T
        final_outputs = np.dot(final_inputs.T,self.weights_hidden_to_output)
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        print("Hiddenerror term", hidden_error_term)
        print("Hiddenouputs", hidden_outputs)
            #could be here that this is the wrong way around because there is no sigmoid there
            
        # Weight step (input to hidden)
        delta_weights_i_h += X[:,None].dot(hidden_error_term.T)
        # Weight step (hidden to output)
        delta_weights_h_o += hidden_outputs.dot(output_error_term)
        print("delta_weights are",delta_weights_h_o)
    ...
    def update_weights(self, delta_weights_i_h, delta_weights_h_o, n_records):
        ...
        print("updated h2o weights are",self.weights_hidden_to_output)
